...
!developmentChains.includes(network.name)
  ...
  : describe('Dynamic SVG NFT Test', () => {
      ...
      describe('constructor', () => {
        it('init values correctly', async () => {
          ...
          assert.equal(priceFeed, dataFeedAddress);
          assert.equal(lowImageUri, lowSVG);
          assert.equal(highImageUri, highSVG);
        });
      });
      describe('mintNft', () => {
        it('update token counter, and emit event, ', async () => {
          const highValue = await ethers.parseEther('1');
        ...
        it('get highTokenURI when eth price is higher than the value user set', async () => {
          const highValue = await ethers.parseEther('1'); // 1 dollar per eth, mocked eth price is 2 dollar per eth
        ...
        it('get lowTokenURI when eth price is lower than the value user set', async () => {
          const highValue = await ethers.parseEther('10'); // 10 dollar per eth, mocked eth price is 2 dollar per eth
          const tx = await DynamicSvgNFT.mintNft(highValue);
          await tx.wait(1);
          const tokenUri = await DynamicSvgNFT.tokenURI(0);
          assert.equal(tokenUri, lowTokenURI);
        });
        it('revert if no token found', async () => {
          await expect(DynamicSvgNFT.tokenURI(123)).to.be.revertedWith(
